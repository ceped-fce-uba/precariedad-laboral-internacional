...
La base de datos homogeneizada de *precariedad.mundial* puede servir de base para el cálculo de un conjunto de indicadores sociolaborales y busca simplificar la comparación entre países. Para procesar la base de datos primero se debe abrir `base_completa.parquet` como dataframe. 

```{r Cargando base}
# Cargar la base de datos homogeneizada
base <- arrow::read_parquet("../base_completa.parquet")
```
...
Paises <- read.xlsx("../fuentes_complementarias/Prod y Salarios.xlsx", sheet = "Paises")
